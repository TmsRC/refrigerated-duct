...
def calculate_geometry(top_profile,bottom_profile):
    P = 2*(top_profile-bottom_profile+w) # perimeter of the (rectangular) cross section of the piece: 2*height + 2*width
    Ac = (top_profile-bottom_profile)*w # area of the (rectangular) cross section of the piece: height*width
...
#         coefficients[i,i] = -8*Ac[i]*k - 2*h*dx**2*P[i] # Naive approximation. First order in error (probably)
        coefficients[i,i] = -8*Ac[i]*k - 2*h*dx*(As[i+1] - As[i-1]) # Gives accurate results, but requires a severe hit in performance
#         coefficients[i,i] = -8*Ac[i]*k - h*dx**2*(P[i+1]+P[i-1])
#         coefficients[i,i] = -8*Ac[i]*k - h*dx**2*(P[i+1]+2*P[i]+P[i-1])/2
...
def calculate_temperature_profile(P,Ac,As):
    coefs = init_coef_matrix(P,Ac,As)
...
def get_profile_efficiency(top_profile,bottom_profile):
    ...
    P,Ac,As = calculate_geometry(top_profile,bottom_profile)
    
    thetas_vec = calculate_temperature_profile(P,Ac,As) # vector of (excess) temperatures along the fin
...
P,Ac,As = calculate_geometry(straight_profile,bottom_profile)
thetas_str = calculate_temperature_profile(P,Ac,As)
...
# P,Ac,As = calculate_geometry(test_profile,bottom_profile)
# thetas = calculate_temperature_profile(P,Ac,As)
